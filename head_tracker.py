...
class HeadTracker(QThread):
    ...
    low_light_detected   = pyqtSignal(bool)   # HTS-ERR-02

    # HTS-ERR-02 — Ortalama parlaklık bu değerin altındaysa kullanıcı uyarılır
    LOW_LIGHT_THRESHOLD = 50   # 0-255 arası (0 = siyah, 255 = beyaz)
    ...
    def __init__(self, parent=None):
        ...
        self._low_light_active      = False   # HTS-ERR-02 spam koruması
    ...
    def run(self):
        # 🔥 MODELİ İŞ PARÇACIĞI İÇİNDE BAŞLAT (Thread-Safety)
        # Mediapipe nesneleri oluşturuldukları thread'de kullanılmalıdır.
        from mediapipe.tasks.python.vision import FaceLandmarkerOptions, FaceLandmarker
        from mediapipe.tasks.python import BaseOptions
        ...
        self.last_time = time.time() # Başlarken süreyi sıfırla
        # 🔥 Windows'ta daha stabil olan DSHOW modunu dene, olmazsa varsayılanı kullan
        cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
        if not cap.isOpened():
            cap = cv2.VideoCapture(0)

        if not cap.isOpened():
            self.error_occurred.emit("Kamera açılamadı! Başka bir uygulama kamerayı kullanıyor olabilir.")
            return
        ...
        while self.is_running:
            try:
                ...
                # HTS-ERR-02 — Düşük ışık tespiti
                gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                brightness = float(np.mean(gray_frame))
                is_low_light = brightness < self.LOW_LIGHT_THRESHOLD
                if is_low_light != self._low_light_active:
                    self._low_light_active = is_low_light
                    self.low_light_detected.emit(is_low_light)

                rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                rgb_frame = np.ascontiguousarray(rgb_frame) # Bellek düzenini garanti et
                
                # 🔥 GÜVENLİ KOPYA: UI thread'i işlerken hafızanın değişmemesi için
                self.frame_processed.emit(rgb_frame.copy())
                ...
                # print("DEBUG: Detect basliyor...")
                result = self.detector.detect(mp_image)
                # print("DEBUG: Detect bitti.")

                if result.face_landmarks:
                    # 1. Yüz bulundu sinyalini SADECE yüz yeni geldiyse gönder
                ...
                print(f"[Tracker Error] Loop hatasi: {e}")

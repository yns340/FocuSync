...
class FocusPage(QWidget):
    ...
    def __init__(self, user_id: int, db_manager, whitelist_page=None, parent=None):
        ...
        self._paused         = False   # FSS-REQ-04/05
        self._elapsed = 0
...
# --- DERS VE SEANS SEÇİM PANELİ 
        ...
        # FSS-REQ-04/05 — Duraklatma / Devam Ettirme butonu
        self.pause_btn = QPushButton("⏸  Duraklatma")
        self.pause_btn.setObjectName("pause_btn")
        self.pause_btn.setFixedSize(220, 50)
        self.pause_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.pause_btn.setEnabled(False)   # Seans başlamadan devre dışı
        self.pause_btn.setFont(self.start_btn.font())
        self.pause_btn.setStyleSheet("""
            QPushButton { background-color: transparent; color: #f59e0b; border: 2px solid #f59e0b; border-radius: 8px; font-weight: bold; }
            QPushButton:hover { background-color: #f59e0b; color: #111318; }
            QPushButton:disabled { color: #4b5563; border-color: #4b5563; }
        """)
    ...
    def _toggle_pause(self):
        """FSS-REQ-04/05: Aktif seansı duraklatır veya devam ettirir."""
        if not self._session_active:
            return
        ...
        if not self._paused:
            ...
            self._paused = True
            self._timer.stop()
            # Kamera iş parçacığını askıya al (is_running koru, sadece durdur)
            if self.tracker and self.tracker.isRunning():
                self.tracker.is_running = False
            self.pause_btn.setText("▶  Devam Et")
            self.notif.show_warning("⏸", "Seans duraklatıldı.", "#f59e0b", 3000)
        else:
            # ─── DEVAM ETTİRME ───
            self._paused = False
            self._timer.start(1000)
            # Kamera iş parçacığını yeniden başlat
            if self.tracker:
                self.tracker.is_running = True
                if not self.tracker.isRunning():
                    self.tracker.start()
            self.pause_btn.setText("⏸  Duraklatma")
            self.notif.show_warning("▶", "Seans devam ediyor!", "#00e5a0", 2000)
    ...
    def _start_session(self):
        ...
        if self._bypass_camera_for_test:
            ...
            return
        """
        # Eski tracker varsa temizle (Çökmeleri önlemek için)
        if self.tracker:
            try:
                self.tracker.is_running = False
        ...
        self.tracker.low_light_detected.connect(self._on_low_light)  # HTS-ERR-02
    ...
    def _end_session(self):
        ...
        # FSS-REQ-04/05 — Pause butonu sıfırla
        ...
        if self.current_focus_session_id and self._elapsed > 0:
            ...
            if self.tracker and self.tracker.total_session_time > 0:
                score = int((self.tracker.total_focus_time / self.tracker.total_session_time) * 100)
                
            # 2. KARAR MOTORUNU VE STUDYPLANS'I TETİKLE (ÖNEMLİ!)
            # Süreleri dakikaya çevirip güncelliyoruz.
            actual_min = self._elapsed / 60.0
            planned_min = self.work_spin.value()
            c_id = self.course_combo.currentData()
            s_id = self.session_combo.currentData()
            
            # Bugünkü günün adını Türkçe olarak alıyoruz
            turkish_days = {"Monday": "Pazartesi", "Tuesday": "Salı", "Wednesday": "Çarşamba", "Thursday": "Perşembe", "Friday": "Cuma", "Saturday": "Cumartesi", "Sunday": "Pazar"}
            day_name = turkish_days.get(datetime.now().strftime('%A'), "Pazartesi")

            # ADG-REQ-02: Gerçek ihlal sayısını ilet
            whitelist_violations = self.current_violations
            self.db_manager.mark_session_completed(
            ...
            # 3. AI Sayfa İçi Güncellemesini tetikle
            # Not: Tracker stop() sinyaliyle on_session_finished_with_ai zaten çağrılacak.
            # Ancak tracker durmuşsa veya sinyal gelmezse diye burada emniyetli çağrılabilir.
            # Şimdilik sinyale güveniyoruz, manuel çağırmayı kaldırıyoruz veya kontrol ekliyoruz.
            pass
    ...
    def _on_frame_processed(self, frame):
        try:
            h, w, ch = frame.shape
            bytes_per_line = ch * w
            # .copy() kullanarak QImage'in worker thread'deki numpy array'inden bağımsız olmasını sağlıyoruz
    ...
    def _on_low_light(self, is_low: bool):
        """HTS-ERR-02: Düşük ışık uyarısı göster / kaldır."""
        if is_low:
            self.notif.show_warning(
                "🌑",
                "Ortam ışığı yetersiz! Kamera yüzünüzü doğru analiz edemeyebilir.",
                "#f59e0b",
                5000,
            )
        else:
            # Eğer banner düşük ışıktan dolayı açıksa kapat
            self.notif.setVisible(False)
    ...
    def on_session_finished_with_ai(self, session_data):
        try:
            ...
            # 1. DEĞİŞKENLERİ EN BAŞTA TANIMLIYORUZ (Çökme olmasın diye)
            score = session_data.get("focus_score", 0)
            selected_course_id = self.course_combo.currentData()
            # Dersin ismini ComboBox'tan dinamik alıyoruz:
            course_name = self.course_combo.currentText().split(" - ")[-1] if " - " in self.course_combo.currentText() else "Ders"
            violations = self.current_violations
            current_work = self.work_spin.value()
            current_break = self.break_spin.value()
            
            # 2. 🔥 KRİZ YÖNETİMİ VE TELAFİ TETİKLEYİCİSİ
            if score < 50:
                print(f"[AI ENGINE] Düşük odaklanma tespit edildi (%{score}). Telafi seansı planlanıyor...")
                from decision_engine import DecisionEngine
                engine = DecisionEngine(self.db_manager)
                engine._inject_makeup_session(self.user_id, selected_course_id, course_name)
    ...
    def _load_recommended_plan(self):
        """Veritabanındaki (StudyPlans) bugüne ait array formatındaki seansları çeker."""
        self.session_combo.clear()
        try:
            # Arkadaşının uyardığı doğru array formatını veritabanından çekiyoruz
            success, plan_data = self.db_manager.get_study_plan(self.user_id)
            if not success or not plan_data:
                self.session_combo.addItem("Henüz plan oluşturulmadı", "none")
                return

            weekly_sessions = plan_data.get("weekly_sessions", {})
            
            # Sadece bugünün planlarını al
            turkish_days = {"Monday": "Pazartesi", "Tuesday": "Salı", "Wednesday": "Çarşamba", "Thursday": "Perşembe", "Friday": "Cuma", "Saturday": "Cumartesi", "Sunday": "Pazar"}
            today_tr = turkish_days.get(datetime.now().strftime('%A'), "Pazartesi")
            
            today_plan = weekly_sessions.get(today_tr, [])
            
            if not today_plan:
                self.session_combo.addItem("Bugün için planlanan ders yok", "none")
                return

            # Array içindeki objeleri ComboBox'a ekle
            for session in today_plan:
                if not session.get("is_completed", False): # Sadece bitmemişleri göster
                    course_name = session.get('course_name', 'Bilinmeyen Ders')
                    start_time = session.get('start_time', '')
                    display_text = f"{start_time} - {course_name}"
                    self.session_combo.addItem(display_text, session.get("session_id"))

            if self.session_combo.count() == 0:
                self.session_combo.addItem("Bugünkü tüm görevler bitti! 🎉", "none")

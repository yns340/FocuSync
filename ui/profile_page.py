...
class ProfilePage(QWidget):
    ...
    def _load_profile(self):
        ...
        if success:
            ...
            self._current_password_hash = data.get("password", "")
            self._current_salt = data.get("salt", "")
        else:
    ...
    def _save_profile(self):
        ...
        if c_pass or n_pass or n_pass_conf:
            ...
            is_valid = False
            if self._current_salt:
                # Kullanıcının girdiği mevcut şifreyi kendi tuzuyla hash'le
                calc_hash, _ = self.db_manager._hash_password(c_pass, self._current_salt)
                if calc_hash == self._current_password_hash:
                    is_valid = True
            else:
                # Eğer eski kayıtsa (tuzu yoksa) düz metin kontrolü yap (Geriye dönük uyumluluk)
                if c_pass == self._current_password_hash:
                    is_valid = True
